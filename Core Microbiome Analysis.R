...
#Acropora
acropora <- subset(Mbon_meta_all, host_taxon_abbreviation=="AC")
...
#Orbicella 
orbicella <- subset(Mbon_meta_all, host_taxon_abbreviation=="OF")
...
#Siderastrea
siderastrea <- subset(Mbon_meta_all, host_taxon_abbreviation=="SS")
...
host_shared_core_microbiome <- inner_join(siderastrea_core_taxa_standard_85, orbicella_core_taxa_standard_85, by=c("Feature.ID", "Sequence", "Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species"))
host_shared_core_microbiome <- inner_join(host_shared_core_microbiome, acropora_core_taxa_standard_85, by=c("Feature.ID", "Sequence", "Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species"))

view(host_shared_core_microbiome)
...
ggvenn(list_venn,
       ...
       text_size = 4.5)
?ggvenn
ggsave("host_core_microbiome_venn.jpeg", plot = last_plot(), device = "jpeg", 
...
my_colors <- values=c("#AE0F1B", "#E69F00", "#56B4E9")
...
ggsave("env_feature_core_microbiome_venn.jpeg", plot = last_plot(), device = "jpeg", 
       width = 6,
       height = 5,
       path = "~/Documents/University of Miami/MBON Project/figures",
       dpi = 300)
